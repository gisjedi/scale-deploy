...
class UniversePackageBuilder(object):
    ...
    def _get_scale_mapping_for_content(self, orig_content):
        '''Returns a scale mapping (dict) for the following cases:
        ...
        - Custom environment params like 'SCALE_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default scale values (may be overridden via eg SCALE_PACKAGE_VERSION envvars):
        scale_mapping = {
        ...
        # look for any 'sha256:filename' scale params, and get shas for those.
        # this avoids calculating shas unless they're requested by the scale.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            scale_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom SCALE_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('SCALE_'):
                # 'SCALE_SOME_KEY' => 'some-key'
                scale_mapping[env_key[len('SCALE_'):].lower(
                ).replace('_', '-')] = env_val

        return scale_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        scale_mapping = self._get_scale_mapping_for_content(orig_content)
        new_content = orig_content
        for scale_key, scale_val in scale_mapping.items():
            new_content = new_content.replace(
                '{{%s}}' % scale_key, scale_val)
        ...
        scale_keys = list(scale_mapping.keys())
        scale_keys.sort()
        for key in scale_keys:
            logger.info('  {{%s}} => %s' % (key, scale_mapping[key]))
...
def print_help(argv):
    logger.info(
        'Syntax: {} <package-name> <package-version> <scale-package-dir> <artifact-base-path> [artifact files ...]'.format(argv[0]))
    logger.info(
        '  Example: $ {} kafka 1.2.3-4.5.6 /path/to/scale/jsons/ https://example.com/path/to/kafka-artifacts /path/to/artifact1.zip /path/to/artifact2.zip /path/to/artifact3.zip'.format(argv[0]))
    logger.info(
        'In addition, environment variables named \'SCALE_SOME_PARAMETER\' will be inserted against the provided package scale (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # local path where the package scale is located:
