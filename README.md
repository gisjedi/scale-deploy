This scale README may be used as a starting point for writing a Service Guide for your DC/OS Service.
...
First, we'll fetch the default application's environment, current application's environment, and the actual scale that maps config values to the environment:
...
```
1. We'll also get the entire application scale:
```bash
...
```
1. Now compare these values to the values contained in the `env` section in application scale:
```bash
